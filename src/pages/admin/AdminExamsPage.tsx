...
export default function AdminExamsPage() {
  ...
  const loadData = async () => {
    const [e, s, c] = await Promise.all([
      supabase.from('exams').select('*, subjects(name), categories(name)').order('created_at', { ascending: false }),
    ...
    if (s.data && s.data.length > 0 && expandedSubject === null) {
      setExpandedSubject(s.data[0].id);
    }
  };
    ...
    <div className="p-6 space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{exams.length} đề — {subjects.length} môn học</p>
        </div>
