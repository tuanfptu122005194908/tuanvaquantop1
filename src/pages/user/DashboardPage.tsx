...
export default function DashboardPage() {
  ...
  const loadData = async () => {
    ...
    if (subjectIds.size > 0) {
      const { data: subjects, count: subjectsCount } = await supabase.from('subjects').select('*', { count: 'exact' })
      ...
      setSubjectsTotal(subjectsCount || 0);
    } else {
    ...
    setStats({ totalAttempts: allAttempts?.length || 0, totalSpent, subjectsCount: subjectIds.size });
    setLoading(false);
  };
